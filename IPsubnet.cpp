...
string binary;
int flaga1, flaga_type, sub_break, decimal;
...
void ToBinary(int decimalToBinary)
{
    ...
    int temp_binary[8]; // maksymalna liczba bit�w potrzebna do zapisu liczby ca�kowitej
    ...
    if ((decimalToBinary != 0) && (decimalToBinary < 255)) {
        ...
        //sprawdzenie d�ugo�ci oktetu
        amount = binary.length();
        //uzupe�nie oktetu o 0 z przodu je�li to potrzbne
        if (amount < 8) {
...
void SubnetByMask() {
    int prefix, subnet_octet, subnet_bits, amount, number_of_octets, choice;
    flaga1 = 0;
